...
def is_a_hand_position_set(sample: DataSample) -> bool:
    return sample.gestures[0].l_hand_position is not None or sample.gestures[0].r_hand_position is not None


def set_hand_position_if_not_set(sample: DataSample) -> None:
...
def decompose_into_static_sample(sample: DataSample, label: str) -> deque[DataSample]:
    """Create len(sample.gestures) new samples
    containing each a different frame of sample repeated len(sample.gestures) times.
    Args:
        sample (DataSample): Sample to decompose
        label (str): The label of the new sub samples

    Returns:
        deque[DataSample]: New sub samples
    """
    new_sub_samples: deque[DataSample] = deque()

    for frame in sample.gestures:
        new_sample: DataSample = DataSample(label=label, gestures=[frame])
...
def gen_dynamic_data(sample: DataSample,
                     nb_frame: int,
